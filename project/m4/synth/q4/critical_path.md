...
## The path (longest topological path = **290** mapped cells)
...
- A comparable path runs `x_chunk/w_row -> 16x 4x4 signed multipliers -> prod_ext_q`.

## Logic stages
| Stage | From -> To | Logic |
|---|---|---|
| 1 multiply | `x_chunk/w_row` -> `prod_ext_q` | 16x parallel 4x4 signed multiply |
| **2 reduce (critical)** | **`prod_ext_q` -> `tree_q`** | **16-input 40-bit adder tree (tree_sum)** |
| 3 accumulate | `tree_q` -> `sum_out` | one 40-bit add |

## Precision effect
The ACC_W=40 adder tree is **width-independent**, so logic depth barely moves across precisions (Q15=307, INT8=296, Q4=290 cells). What shrinks with DATA_W is **area** (multiplier area ~ DATA_W^2): this lane is 2799 cells / 21180 um^2. The design is depth-bound by the fixed-width accumulator, not the multiplier width.
